...
function! hz#bufferize#do(command) abort
  if a:command ==# '' | return | endif

  redir => l:output | silent execute a:command | redir END
  ...
  if l:bfz_buffer != 0
    ...
    if l:bfz_buffer < 0 | execute 'sbuffer ' . abs(l:bfz_buffer) | endif
  ...
  if l:at_last_line | normal! G | endif

  if exists(':RunCommand') == 2 | execute 'RunCommand silent Bufferize ' . a:command | endif

  if l:bfz_buffer < 0 | execute bufwinnr(abs(l:bfz_buffer)) . 'wincmd c' | endif
...
function! hz#bufferize#bufnr(command) abort
  ...
  for l:bufnr in tabpagebuflist()
    if bufname(l:bufnr) =~? l:name | return l:bufnr | endif
  endfor

  let l:bufnr = bufnr(l:name)

  if l:bufnr == -1 | return 0 | endif
